...
async def get_credential(cred_id: UUID, user: CurrentUser = None, service: VaultService = Depends(get_service)):
...
async def create_credential(body: CredentialCreate, user: CurrentUser = None, service: VaultService = Depends(get_service)):
    return await service.create_credential(body, user.profile_id)
...
async def update_credential(cred_id: UUID, body: CredentialUpdate, user: CurrentUser = None, service: VaultService = Depends(get_service)):
    return await service.update_credential(cred_id, body, user.profile_id)


@router.delete("/{cred_id}", status_code=204)
async def delete_credential(cred_id: UUID, user: CurrentUser = None, service: VaultService = Depends(get_service)):
