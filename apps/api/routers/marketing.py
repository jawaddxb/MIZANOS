...
async def list_domains(product_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.get_domains(product_id)


@router.post("/domains", response_model=DomainResponse, status_code=201)
async def create_domain(body: DomainCreate, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.create_domain(body)


@router.patch("/domains/{domain_id}", response_model=DomainResponse)
async def update_domain(domain_id: UUID, body: DomainUpdate, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.update_domain(domain_id, body)


@router.delete("/domains/{domain_id}", status_code=204)
async def delete_domain(domain_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
    await service.delete_domain(domain_id)


@router.get("/social-handles", response_model=list[SocialHandleResponse])
async def list_social_handles(product_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
...
async def create_social_handle(body: SocialHandleCreate, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.create_social_handle(body)


@router.patch("/social-handles/{handle_id}", response_model=SocialHandleResponse)
async def update_social_handle(handle_id: UUID, body: SocialHandleUpdate, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.update_social_handle(handle_id, body)


@router.delete("/social-handles/{handle_id}", status_code=204)
async def delete_social_handle(handle_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
    await service.delete_social_handle(handle_id)


@router.get("/checklist", response_model=list[ChecklistItemResponse])
async def list_checklist(product_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.get_checklist(product_id)


@router.post("/checklist", response_model=ChecklistItemResponse, status_code=201)
async def create_checklist_item(body: ChecklistItemCreate, user: CurrentUser, service: MarketingService = Depends(get_service)):
...
async def apply_template(body: ApplyTemplateRequest, user: CurrentUser, service: MarketingService = Depends(get_service)):
...
async def toggle_checklist_item(item_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
...
async def create_credential(body: CredentialCreate, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.create_credential(body)


@router.get("/credentials", response_model=list[CredentialResponse])
async def list_credentials(product_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
...
async def decrypt_credential(credential_id: UUID, user: CurrentUser, service: MarketingService = Depends(get_service)):
    return await service.decrypt_credential(credential_id)


@router.post("/auto-populate", response_model=AutoPopulateResponse)
async def auto_populate(body: AutoPopulateRequest, user: CurrentUser, service: MarketingService = Depends(get_service)):
