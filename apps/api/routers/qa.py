...
async def create_qa_check(body: QACheckCreate, user: CurrentUser = None, service: QAService = Depends(get_service)):
    return await service.create_check(body)


@router.patch("/{check_id}", response_model=QACheckResponse)
async def update_qa_check(check_id: UUID, body: QACheckUpdate, user: CurrentUser = None, service: QAService = Depends(get_service)):
...
async def generate_qa_checklist(product_id: UUID, user: CurrentUser = None, service: QAService = Depends(get_service)):
