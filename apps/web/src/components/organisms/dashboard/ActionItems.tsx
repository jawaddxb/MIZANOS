      ...
      <Card className="h-full">
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <div className="h-12 w-12 rounded-full bg-status-healthy/10 flex items-center justify-center mb-3">
              <CheckCircle2 className="h-6 w-6 text-status-healthy" />
            </div>
            <p className="font-medium text-foreground">All clear!</p>
            <p className="text-sm text-muted-foreground mt-1">No action items at the moment</p>
          </div>
        </CardContent>
      </Card>
    );
            ...
            {actionItems.slice(0, 12).map((item, index) => {
              ...
              return (
                <Link
                  ...
                  className={cn("block p-3 rounded-lg border transition-all duration-200 group", styles.border, styles.bg)}
                  style={{ opacity: 0, animation: `fade-in 0.2s ease-out ${index * 50}ms forwards` }}
                >
                  <div className="flex items-start gap-3">
                    <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover:scale-110", styles.icon)}>
                      {iconMap[item.type]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium text-foreground truncate">{item.title}</p>
                        <ChevronRight className="h-4 w-4 text-muted-foreground/50 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" />
                      </div>
                      ...
                      {item.meta && <p className="text-xs text-muted-foreground/70 mt-0.5">{item.meta}</p>}
                    </div>
                  </div>
                </Link>
              );
